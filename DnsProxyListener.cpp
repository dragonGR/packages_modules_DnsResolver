...
bool synthesizeNat64PrefixWithARecord(const netdutils::IPPrefix& prefix, addrinfo** res,
    ...
    // Build a synthesized AAAA addrinfo list from the queried A addrinfo list.
    for (const addrinfo* cur4 = head4; cur4; cur4 = cur4->ai_next) {
        // Allocate a space for a synthesized AAAA addrinfo. Note that the space of addrinfo and
        // sockaddr{*} are allocated continuously because freeaddrinfo() releases all space of
        // addrinfo and its sockaddr{*} by freeing the start address of addrinfo once.
    ...
    // AF_UNSPEC is specified. Prefer that IPv4-embedded IPv6 addresses have higher precedence than
    ...
    // synthesized.
    // TODO: Perhaps re-sort the concatenated addresses by RFC 6724 section 2.1. Note that the
